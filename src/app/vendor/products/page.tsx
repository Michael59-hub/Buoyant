...
import { ActiveToggleDropdownItem, DeleteDropdownItem } from "@/app/admin/products/_components/productAction"
        ...
        {products.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-24 border border-white/8 rounded-2xl bg-white/[0.02]">
            <div className="w-14 h-14 rounded-2xl bg-white/5 flex items-center justify-center mb-4">
              <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                <path d="M20 7H4a2 2 0 00-2 2v10a2 2 0 002 2h16a2 2 0 002-2V9a2 2 0 00-2-2zM16 3H8L6 7h12l-2-4z" stroke="white" strokeWidth="1.5" strokeOpacity=".3" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </div>
            <p className="text-white/30 text-sm mb-4">No products yet</p>
            <Link href="/vendor/products/new" className="inline-flex items-center gap-2 bg-[#c8f533] text-black font-bold px-5 py-2.5 rounded-full text-sm hover:bg-white transition-colors">
          ...
          <div className="border border-white/8 rounded-2xl overflow-hidden bg-white/[0.02]">
            ...
            <div className="grid grid-cols-[auto_1fr_auto_auto_auto_auto] items-center gap-4 px-6 py-3 border-b border-white/8 bg-white/[0.02]">
              ...
              <span className="text-xs font-mono text-white/25 uppercase tracking-widest w-16 text-right">Sales</span>
            ...
            <div className="divide-y divide-white/5">
              {products.map((product) => (
                <div key={product.id} className="grid grid-cols-[auto_1fr_auto_auto_auto_auto] items-center gap-4 px-6 py-4 hover:bg-white/[0.025] transition-colors group">
                  ...
                  <div className="w-20 text-right">
                    <span className="text-[#c8f533] font-bold text-sm font-mono">{formatCurrency(product.price / 100)}</span>
                  </div>
                  <div className="w-16 text-right">
                    <span className="text-white/40 text-sm font-mono">{formatNumber(product._count.orders)}</span>
                  </div>
                  <div className="w-20 flex justify-center">
                    {product.isAvailableForPurchase ? (
                      <span className="text-[10px] font-mono font-bold text-[#c8f533] bg-[#c8f533]/10 border border-[#c8f533]/20 px-2 py-0.5 rounded-full uppercase tracking-wider">Live</span>
                    ) : (
                      <span className="text-[10px] font-mono font-bold text-white/25 bg-white/5 border border-white/10 px-2 py-0.5 rounded-full uppercase tracking-wider">Hidden</span>
                    )}
                  </div>
            ...
            <div className="px-6 py-3 border-t border-white/8 bg-white/[0.015] flex items-center justify-between">
              <p className="text-xs text-white/20 font-mono">{products.length} product{products.length !== 1 ? "s" : ""} total</p>
